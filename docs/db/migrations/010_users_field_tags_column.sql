...
-- field 컬럼은 011_drop_legacy_field.sql이 field_legacy_removed로 이름을 바꿔 이미 없어졌을 수
-- 있다. 그 상태에서 운영 DB에 이 스크립트를 실수로 재적용하면 COMMENT ON COLUMN이 존재하지
-- 않는 컬럼을 대상으로 실패해 재실행 안전성이 깨지므로, 컬럼이 남아있을 때만 코멘트를 단다.
DO $$
BEGIN
    IF EXISTS (
        SELECT 1 FROM information_schema.columns
