...
- **`MeetingAnalysisService`** (기존 빈, 축소)
  - `analyze(...)`: 동기 트랜잭션. 텍스트 추출 → `Meeting` 저장(`analysis_status=processing`) → 업로드 파일 저장 → 참석자 저장 → 커밋 이후 `MeetingAnalysisRunner.runAnalysis(meetingId, request)` 실행 예약 → 즉시 응답(`status=PROCESSING`, `analysis=null`) 반환.
...
- **`MeetingAnalysisPersistence`** (저장 전담 빈)
  - `saveAnalysisSuccess(meetingId, result, analysisSource)` — `@Transactional`. `MeetingAnalysis` 저장 + `MeetingActionItem` 목록 저장 + RAG 색인 best-effort 호출 + `Meeting.analysisStatus=completed`.
  - `saveAnalysisFailure(meetingId, errorMessage)` — `@Transactional`. `Meeting.analysisStatus=failed` + 안전한 `analysisErrorMessage` 저장.
...
- 초기 저장(파일/참석자/`Meeting`)은 `analyze()` 트랜잭션 안에서 원자적으로 처리한다.
- 비동기 분석 스레드는 `TransactionSynchronization.afterCommit()`에서 시작해, 커밋 전 `processing` 회의록을 조회하는 레이스를 방지한다.
- 성공/실패 저장은 `MeetingAnalysisPersistence`의 별도 `@Transactional` 메서드로, 다른 빈(`MeetingAnalysisRunner`)에서 호출되므로 프록시가 정상 적용된다.
