...
--
-- ⚠️ 파괴적 구문(RENAME/DROP) 비활성화 상태: docs/db/migrations/011_drop_legacy_field.sql(운영
-- DB에 수동 적용하는 동일 스크립트)이 App/DEPLOY_OCI.md 8-1절 결정에 따라 보류돼 있는 것과
-- 100% 동일한 스키마 상태를 유지하기 위해, 신규 DB를 만드는 이 스크립트도 같은 이유로
-- 비활성화한다. 신규 DB와 운영 DB가 다른 시점(하나는 rename됨, 하나는 안 됨)에 놓이면
-- 스키마가 갈라지기 때문이다. 모든 인스턴스가 field_tags 기반 코드로 교체됐다고 확인된 뒤,
-- 이 파일과 docs/db/migrations/011을 함께 재활성화할 것 — 아래 ALTER TABLE 구문의 주석을
-- 해제하면 된다. 그 전까지 이 스크립트는 아무 것도 변경하지 않는 안전한 no-op이다.
--
...
-- (재활성화해서) 먼저 실행하면 안 된다. 모든 인스턴스가 field_tags 기반 코드(현재 버전)로
-- 교체된 뒤에만 실행할 것.
--
-- 재실행 안전성: 초기화 스크립트 전체를 재실행하면 04(ADD COLUMN IF NOT EXISTS)가 field를
-- 다시 만들어낸 뒤 이 스크립트가 또 돌 수 있다. 재활성화 시 field_legacy_removed가 이미 있어
